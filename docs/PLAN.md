...
- [ ] 3.4 **[OWNER-GATED] Nexo Short consumes it (coordinated)** — FIRST: deploy the 3.3 oauth-uuid corrective migration to production (`migrate --force`; safe — oauth codes/tokens there are smoke-test leftovers at most). Then register Nexo Short's production client (`nexo:sso-client`, exact `nxo.li` redirect URIs) on the server; support that project's agent integrating the template per its own plan; never edited unilaterally from here.
