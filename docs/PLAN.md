...
- [x] 3.1 **[OWNER-GATED] T4 ops pass** — done 2026-07-21/22: nightly MySQL dumps (auto-discovers apps via `.env` scan, 14-day rotation) + off-site sync to Google Drive (rclone) + healthchecks.io heartbeat + restore tested locally (nexoid dump → full verify) + UptimeRobot on all three `/up` endpoints. Cron created in hPanel by Alvaro (03:15); runbook + script in standards repo `inbox/nexoid/`. Owner closed it 2026-07-22; first unattended cron run self-confirms via healthchecks (alerts if not). Maintenance note: own rclone client_id before Google retires the shared one (late 2026).
- [x] 3.2 **Client pattern template** — built 2026-07-21 in `templates/nexo-sso-client/` (standards repo): OIDC client (cached discovery, PKCE S256 + state, id_token validation vs JWKS), `NEXO_SSO_*` config, `nexo_id_sub` migration, takeover-safe account linking, degradation, README, Pest suite (16 tests, 12/12 ACs name-traced, grep sweep 1:1; `php -l` clean). Runtime proof against a real provider is task 3.3.
- [x] 3.3 **Prove the template against a real local Nexo ID** — done 2026-07-21/22: throwaway consumer app + full e2e over real HTTP (register→verify→silent-SSO authorize→code→PKCE token exchange→id_token vs real JWKS→local user linked by sub→session survives provider down). Template suite 16/16 in the consumer; sweep 1:1. **Found & fixed a real production-affecting bug:** Passport's published migrations kept `user_id`/`owner_id` as bigint while users are uuid — invisible on sqlite, breaks (or silently truncates) on MySQL/MariaDB. Corrective migration added; **deployed to production 2026-07-22 (verified via `migrate:status`; see task 3.4)**. Also: local http providers need `OPENID_FORCE_HTTPS=false`; the bridge signs id_tokens without `kid` (client made tolerant).
- [ ] 3.4 **[OWNER-GATED] Nexo Short consumes it (coordinated)** — nexoid half DONE 2026-07-22 by Alvaro: oauth-uuid migration deployed to production (`migrate:status` verified) and production client registered. **Redirect corrected 2026-07-22:** the callback lives on Nexo Short's PANEL host (`https://nexoshort.alvarocdev.com/auth/nexo/callback`) — `nxo.li` is cookieless redirects-only (that project's ADR-001), so it can't hold the OIDC state/verifier session. Nexo Short's integration is code-complete (SSO dark, `NEXO_SSO_ENABLED=false` until its launch phase); task closes when the real flow works from the panel host.
- [x] 3.5 **Pre-public cleanups** — done 2026-07-22: `DEPLOYMENT.md` genericized (placeholders, no real hosting layout; client-registration example corrected to `/auth/nexo/callback`); `nexo-id.md` moved to the standards repo (owner decision; stays in git history, not sensitive); SEO-base shipped (home meta description/canonical/OG i18n'd, `noindex` on auth pages, `/robots.txt` route with private surface disallowed, cached `/sitemap.xml`, static `public/robots.txt` removed so the route wins; 4 tests in `SeoBaseTest`); hreflang deliberately skipped — locales share one URL (`?lang=` + session), so per-locale alternates don't apply (siblings do the same); author email re-confirmed OK for public.
- [ ] 3.6 **[OWNER-GATED] Repo goes public** — `audit-open-source` full pass CLEAN 2026-07-22 (history: no secrets, only `.env.example` ever tracked; HEAD: no hosting account/IPs/server hostname, `.env.example` valueless, no binaries, Passport keys covered by `/storage/*.key`; author email owner-accepted; CI leak-free; accepted-by-owner notes: `nexo-id.md` stays in history, public-instance URLs intentional). Remaining: Alvaro flips visibility on GitHub (+ update the repo description, still says "Planning phase").

**Gate 3 (owner sign-off required):** real signup→login→use flow on Nexo Short via Nexo ID from its panel host (`nexoshort.alvarocdev.com` — `nxo.li` serves redirects only, per that project's ADR-001); degradation verified (Nexo ID down → active sessions keep working); T4 in place (backups restore-tested + uptime alerts live); audit passed; repo public; owner sign-off.
