...
- [ ] 3.4 **[OWNER-GATED] Nexo Short consumes it (coordinated)** — nexoid half DONE 2026-07-22 by Alvaro: oauth-uuid migration deployed to production (`migrate:status` verified) and production client registered (`Nexo Short` → exact `https://nxo.li/auth/nexo/callback`). Remaining: Nexo Short's agent integrates the template per its own plan (never edited unilaterally from here); task closes when the real flow works from `nxo.li`.
