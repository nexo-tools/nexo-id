...
const TOKEN_EXCEPTIONS = [
    // Passport's own tables, upstream's shape (DATABASE-STANDARD.md §12).
    //
    // oauth_clients.secret IS hashed — Passport hashes it with the app's hasher
    // before persisting; the column simply predates our *_hash naming and is
    // read by the package, so renaming it would fork the dependency.
    'oauth_clients.secret',
    // Not a secret at all: a foreign key to oauth_access_tokens whose name ends
    // in "token_id" because the row it points at is a token.
