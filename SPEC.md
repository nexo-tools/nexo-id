...
- **2026-07-20 (task 1.3)** — Base UI language is **English** (source strings are the keys), a deliberate deviation from the siblings' Spanish base, appropriate for a public/self-hostable identity service with English docs (ADR-001). es/pt are generated maps. No AC changes.
- **2026-07-20 (task 1.6)** — Framework messages (Laravel `validation.*` / `auth.*`) resolve from vendor in **English only**; translated es/pt validation/auth messages (laravel-lang territory) are **deferred to backlog**. No AC requires them (AC-I18N-1/2 cover UI-string locale resolution and sync). Recorded so it is a known gap, not a silent one.
